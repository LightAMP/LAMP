...
class PLOT(object):
    ...
    def __init__(self,X,Y,Descriptions,X_label,Y_label,name,Left=True,Right =False,Up=True,Down=False,condition = False):        
        self.X = X
        self.Y = Y
        self.Left = Left
        self.Right = Right
    ...
    def scatter_line(self,Grid,y,Log = False):
        ...
        legend_elements = []
       # DD = ['LARMIX'+r'$\tau=$'+str(0.6),'LARMIX'+r'$\tau=$'+str(0.3),'Proportional','LARMIX'+ r'$\tau=$' + str(0.9),'Uniform'] 
       # CC = ['fuchsia','cyan','green','red','royalblue']

        for i in range(len(self.Desc)):
            #legend_elements.append(Line2D([0], [4], color=CC[i], lw=4, label=DD[i]))
            legend_elements.append(Line2D([0], [4], color=self.colors[i], lw=4, label=self.Desc[i],linestyle=self.Line_style[i]))
                           

        # Create the figure
        fig, ax = plt.subplots()
        ax.legend(handles=legend_elements,fontsize = 12, loc = self.Place) 
        if Log:
            plt.xscale("log")           
        if Grid:
            plt.grid(linestyle='--')   
    ...
    def Box_Plot(self, yy, Grid): 
        ...
        axs.legend(handles=legend_elements,fontsize = 12, loc = 'lower left') 
        if Grid:
            plt.grid(linestyle='--')
        #plt.ylim(0,12)   
        plt.xlabel(self.XL,fontdict = font1)
    ...
    def scatter3(self,Grid,Log = False):
        ...
        for i in range(3):
            ...
            legend_elements2.append(Line2D([0], [4], marker = self.markers[i+3], color=self.colors[i+3], lw=4, label=self.Desc[i+3],linestyle=self.Line_style[i]))                           

        # Create the figure
        fig, ax = plt.subplots()
        ax2 = ax.twinx()
        ax.legend(handles=legend_elements1,fontsize = 10, loc = 'upper left') 
        ax2.legend(handles=legend_elements2,fontsize = 10, loc = 'upper right')        
        ...
        # create the second scatter plot and set the y-axis label
            ...
            ax2.plot(self.X,self.Y[1][i],  alpha=1,color=self.colors[i+3],linestyle=self.Line_style[i])        
